...
//!   cargo test -p syntecnia-stdlib --test acme_pebble -- --ignored --nocapture
//!
...
use syntecnia_stdlib::server::{self, ServeRuntime};
...
fn acme_http01_end_to_end_against_pebble() {
  ...
  "pebble": {{
    ...
    std::env::set_var("SYNTECNIA_ACME_DIRECTORY", format!("https://127.0.0.1:{}/dir", dir_port));
    std::env::set_var("SYNTECNIA_ACME_CA", &dir_cert_path);
    std::env::set_var("SYNTECNIA_CERT_DIR", &certdir);

    // (7) Emite el cert end-to-end (cuenta → orden → HTTP-01 → finalize → cert).
    let (cert_path, key_path) =
        syntecnia_stdlib::acme::obtain_and_save("127.0.0.1", Some("admin@example.com"), store.clone())
    ...
    std::env::remove_var("SYNTECNIA_ACME_DIRECTORY");
    std::env::remove_var("SYNTECNIA_ACME_CA");
