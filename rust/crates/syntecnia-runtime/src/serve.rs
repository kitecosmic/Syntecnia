...
use syntecnia_agents::swarm::Swarm;
use syntecnia_capabilities::model::{Capability, CapabilitySet, CapabilityType};
use syntecnia_core::ast::{Node, NodeKind};
use syntecnia_core::interpreter::{Control, Interpreter, RunResult, RuntimeError, ServeHook};
use syntecnia_core::number::Number;
use syntecnia_core::parser::{parse_source, CompileError};
use syntecnia_core::types::{
    from_send, syn_map, syn_nothing, syn_text, to_send, SendValue, SynTaskValue, SynValue,
};
use syntecnia_stdlib::acme;
use syntecnia_stdlib::database::{register_database_builtins, DatabaseManager};
...
fn resolve_rate(
    ...
    env: &Rc<RefCell<syntecnia_core::interpreter::Environment>>,
    clause: Option<&Node>,
...
fn make_serve_hook(
    ...
    Rc::new(move |interp, node, env| {
        ...
        if tls_auto {
            ...
            let http_port: u16 = std::env::var("SYNTECNIA_ACME_HTTP_PORT")
                .ok()
...
/// (como `syntecnia run`); `secure=true` para el path seguro (body 500 genérico).
