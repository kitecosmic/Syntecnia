//! Parser de Syntecnia — transforma tokens en AST.
//!
//! Port fiel de `syntecnia/core/parser.py`. Descenso recursivo con precedencia
    ...
    fn expect_name(&mut self, what: &str) -> Result<Token, ParseError> {
        ...
        if keyword_lookup(&tok.raw).is_some() {
            return Err(ParseError::new(
                format!(
                    "'{}' is a reserved word in Syntecnia; choose another name for the {}",
                    tok.raw, what
                ),
                tok.location,
            ));
...
mod tests {
    ...
    fn reserved_word_as_name() {
        ...
        assert!(
            err.to_string().contains("'task' is a reserved word in Syntecnia"),
            "got: {}",
            err
        );
    }
