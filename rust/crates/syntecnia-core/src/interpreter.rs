//! Intérprete de Syntecnia — evalúa el AST.
//!
//! Port fiel de `syntecnia/core/interpreter.py`. El control de flujo `give`/`stop`
...
Intent is frozen to prevent prompt injection from expanding the mandate.",
    ...
    fn b_render(&mut self, _args: &[SynValue], _loc: &SourceLocation) -> Result<SynValue, Control> {
        Err(err("render is only available through the Syntecnia engine runtime"))
    }
...
/// Ejecuta un programa Syntecnia y devuelve su salida observable. Corre en un
